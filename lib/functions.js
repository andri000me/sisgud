...
function hapusMutasi(kode) {
    var url = baseUrl + 'gudang/hapus_mutasi/'+ kode;
    $('#dialog-login-confirm').dialog({
        ...
        buttons: {
            Login: function() {
                ...
                var user = $('#spv_user').val();
                var passwd = $('#spv_passwd').val();
                $.post(
                    url,
                    {'username': user, 'password': passwd},
                    function(data){
                        if (data == 1) {
                            if (confirm ('Mutasi masuk dengan kode: '+kode+' telah dihapus')) {
                                $('#dialog-login-confirm').dialog('close');
                                window.location.reload();
                            }
                        }
                        else {
